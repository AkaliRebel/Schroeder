...
//==============================================================================
const juce::String SchroederReverbAudioProcessor::roomSizeParamID = "roomSize";
const juce::String SchroederReverbAudioProcessor::dampingParamID = "damping";
const juce::String SchroederReverbAudioProcessor::wetLevelParamID = "wetLevel";
const juce::String SchroederReverbAudioProcessor::dryLevelParamID = "dryLevel";
...
SchroederReverbAudioProcessor::SchroederReverbAudioProcessor()
     ...
     : AudioProcessor (BusesProperties()
                       ...
                       .withOutput ("Output", juce::AudioChannelSet::stereo(), true)
                           ...
                           {
                               std::make_unique<juce::AudioParameterFloat>(roomSizeParamID, "Room Size", 0.0f, 1.0f, 0.5f),
                               std::make_unique<juce::AudioParameterFloat>(dampingParamID, "Damping", 0.0f, 1.0f, 0.5f),
                               std::make_unique<juce::AudioParameterFloat>(wetLevelParamID, "Wet Level", 0.0f, 1.0f, 0.5f),
                               std::make_unique<juce::AudioParameterFloat>(dryLevelParamID, "Dry Level", 0.0f, 1.0f, 0.5f)
                           })
#endif
{
}
...
void SchroederReverbAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    auto roomSize = parameters.getRawParameterValue(roomSizeParamID)->load();
    auto damping = parameters.getRawParameterValue(dampingParamID)->load();
    auto wetLevel = parameters.getRawParameterValue(wetLevelParamID)->load();
    auto dryLevel = parameters.getRawParameterValue(dryLevelParamID)->load();

    reverb.setParameters(roomSize, damping, wetLevel, dryLevel);
